...
# Make architecture figure
Pdbs are saved in `data/compare-architectures/pdbs`.

The included sequences are selected from `data/mrbayes/all-seeds-0619/clades/all-linsi.fa` and included in the file `data/compare-architectures/aligned-sequences.fa`

An alignment without columns with all gaps is made by running: `seqconverter -I fasta -O fasta --remallgapcols data/compare-architectures/aligned-sequences.fa > data/compare-architectures/aligned-sequences-noallgaps.fa`.

The included are sequences, along with their discontinuous intervals, are listed in the script `src/compare-architectures/run-dssp.py` and this script is run in order to generate the dssp tables (saved in `data/compare-architectures/architecture-tables-dssp`).

Included structures are listed in the script: `src/compare-architectures/make-architecture-data.py`.

The architecture data is generated by running `python src/compare-architectures/make-architecture-data.py`.

The color file is made manually: `data/compare-architectures/not-aligned-new/name2color.js`.

The architecture figure is saved in `data/compare-architectures/not-aligned-new/arch.html`.