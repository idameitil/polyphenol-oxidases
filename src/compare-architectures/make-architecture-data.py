...
for i in range(len(entries)):
    ...
    for position in positions:
        entries[i]["conserved_positions_alignment"][sequencePos2alignedPos[position["pos"]] + entry["domain_start_structure"] - 1] = {
            'position': position["pos"] + entry["domain_start_structure"] - 1,
            'amino_acid': position["AA"]
            }
    if i == 1:
